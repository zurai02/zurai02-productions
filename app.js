...
function detect(){const e=['syn','krnl','fluxus','getexecutorname','getgc','getconnections','getloadedmodules'];S.isBrowser=!e.some(x=>typeof window[x]!=='undefined');}

// Browser redirect: if opened in browser, go to protection.html
function checkBrowserRedirect(){detect();if(S.isBrowser){window.location.replace('protection.html');}}
...
// Root-level loadstring: raw.html is at root
function genLoadstring(id){const base=window.location.origin+window.location.pathname.replace('index.html','');return'loadstring(game:HttpGet("'+base+'raw.html?script='+id+'",true))()';}
...
function logExec(sid,ok,msg){S.execs.push({sid,t:new Date().toISOString(),ok,msg,env:'executor'});save();}

function render(){const g=document.getElementById('scriptGrid'),e=document.getElementById('empty');if(!S.scripts.length){g.innerHTML='';e.style.display='block';return;}e.style.display='none';g.innerHTML=S.scripts.map(s=>{const le=s.last?new Date(s.last).toLocaleString():'Never';const out=s.output?`<div class="output active">${esc(s.output)}</div>`:'';const lsbox=`<div class="loadstring-box active"><label>📋 Copy this loadstring into your executor</label><div class="loadstring-text">${esc(s.ls)}</div></div>`;return`<div class="card"><div class="card-head"><div class="card-name">${esc(s.name)}</div><div class="card-lang">Lua</div></div><div class="card-desc">${esc(s.desc)}</div><div class="card-meta"><span>▶️ ${s.execs} executions</span><span>🕐 ${le}</span></div><div class="card-actions"><button class="run" onclick="runScript('${s.id}')">▶️ Run</button><button class="copy-btn" onclick="copyLoadstring('${s.id}')">📋 Copy</button><button class="del" onclick="delScript('${s.id}')">🗑️</button></div>${lsbox}${out}</div>`;}).join('');}
...
function load(){try{const sc=localStorage.getItem('zp_scripts'),ex=localStorage.getItem('zp_execs');if(sc)S.scripts=JSON.parse(sc);if(ex)S.execs=JSON.parse(ex);S.scripts.forEach(s=>{if(!s.ls)s.ls=genLoadstring(s.id);if(!s.lang)s.lang='lua';});render();updateStats();}catch(e){}}

function esc(t){const d=document.createElement('div');d.textContent=t;return d.innerHTML;}
function toast(msg,type='info'){const c=document.getElementById('toasts'),t=document.createElement('div');t.className='toast '+(type==='ok'?'ok':type==='err'?'err':'');t.innerHTML=(type==='ok'?'✅':type==='err'?'❌':'ℹ️')+' '+esc(msg);c.appendChild(t);setTimeout(()=>{t.style.opacity='0';t.style.transform='translateX(120%)';setTimeout(()=>t.remove(),300);},4000);}

document.addEventListener('DOMContentLoaded',()=>{checkBrowserRedirect();initParticles();load();document.getElementById('modal').addEventListener('click',e=>{if(e.target===document.getElementById('modal'))closeModal();});document.addEventListener('keydown',e=>{if(e.key==='Escape')closeModal();if(e.ctrlKey&&e.key==='n'){e.preventDefault();openModal();}});document.querySelectorAll('.nav a').forEach(a=>{a.addEventListener('click',()=>{document.querySelectorAll('.nav a').forEach(x=>x.classList.remove('active'));a.classList.add('active');});});});
